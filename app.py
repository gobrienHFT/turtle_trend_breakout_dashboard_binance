...
REQUESTS_PER_SEC = float(os.environ.get("REQUESTS_PER_SEC", "1.2"))
...
st.set_page_config(page_title="Binance Perp Daily Breakouts", layout="wide")
st.title("Binance Perp Daily Breakouts")
st.caption(f"Build: {APP_BUILD}")
st.info(
    "Manual scan only. No auto-refresh. Only daily 90D/180D high/low breakout flags."
)
...
with st.form("run_scan"):
    ...
    quotes = c1.multiselect(
        ...
        default=DEFAULT_QUOTES or ["USDT"],
    )
    max_symbols = c2.slider("Max symbols to scan", min_value=5, max_value=80, value=DEFAULT_MAX_SYMBOLS, step=5)
    buffer_pct = c3.number_input(
        "Breakout buffer (%)",
        min_value=0.0,
        max_value=3.0,
        value=DEFAULT_BUFFER_PCT,
        step=0.01,
        format="%.2f",
    )
    ...
    st.caption(
        "Request budget per scan ≈ 2 fixed calls (exchangeInfo + ticker/price) + 1 daily-kline call per selected symbol."
    )
...
if run_scan:
    try:
        with st.spinner("Fetching symbols/prices and daily levels..."):
            ...
            symbols = tuple(syms["symbol"].tolist())

            prices = get_prices(BASE_URL, HTTP_TIMEOUT, REQUESTS_PER_SEC, RETRIES)
            prices = prices[prices["symbol"].isin(symbols)].copy()
            if prices.empty:
                st.warning("No prices available for selected symbols.")
                st.stop()

            levels = get_levels(BASE_URL, HTTP_TIMEOUT, REQUESTS_PER_SEC, RETRIES, symbols, LEVELS_FETCH_DAYS)
            if levels.empty:
                st.warning("No symbols had enough completed daily history for 90D/180D.")
                st.stop()

            df = syms.merge(prices, on="symbol", how="inner").merge(levels, on="symbol", how="inner")
            df = apply_breakout_flags(df, buffer_pct=float(buffer_pct))
...
mc1.metric("Symbols", stats["symbols"])
mc2.metric("> 90D High", stats["break_90_high"])
mc3.metric("< 90D Low", stats["break_90_low"])
mc4.metric("> 180D High", stats["break_180_high"])
mc5.metric("< 180D Low", stats["break_180_low"])
st.caption(f"Last scan (UTC): {st.session_state.get('scan_ts', '')}")

if out.empty:
    st.warning("No current breakouts for selected universe.")
    st.stop()
